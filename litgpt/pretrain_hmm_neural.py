...
def setup(
    ...
    logger = choose_logger(
        logger_name, out_dir, name=train.wandb_project_name, resume=resume, log_interval=train.log_interval
    )
...
def main(
    ...
    with fabric.init_module(empty_init=True):
        model = GPT(config, n_hidden_states=train.n_hidden_states)
    ...
    if train.max_seq_length:
        model.max_seq_length = train.n_hidden_states
    ...
    # convert hidden state text to the tokens. this will initialize hidden state embeddings
    # load the hidden state text
    with open(train.hidden_state_text, "r") as f:
        hidden_state_text = f.read()
    hidden_state_tokens = tokenizer.encode(hidden_state_text).unsqueeze(0) # [1, ?]
    hidden_state_tokens = hidden_state_tokens[:, :train.n_hidden_states] # [1, hidden_state_tokens]
    assert hidden_state_tokens.size(1) == train.n_hidden_states
    
    fit(fabric, devices, state, train_dataloader, val_dataloader, out_dir, tokenizer_dir, train, eval, hidden_state_tokens)
...
def forward_algo(em, tm, p, input_ids):
    """
    em [1, h, v]
    tm [1, h, h]
    p [1, h, 1]
    ...
    em, tm, p = lsm(em), lsm(tm), lsm(p)
    em = em.squeeze(0)
    tm = tm.repeat(b, 1, 1)
    p = p.repeat(b, 1)
    
    alpha = torch.zeros(b, h, s, device=em.device)
    alpha[:, :, 0] = p + em[:, input_ids[:, 0]].T
    for i in range(1, s):
        # fill in remaining matrix in parallel
        alpha[:, :, i] = em[:, input_ids[:, i]].T + torch.logsumexp(alpha[:, :, i-1].unsqueeze(-1) + tm, dim=-1)

    # sum over the last dimension to get log likelihood
    loglike = torch.logsumexp(alpha[:, :, -1], dim=-1)
    # negative log likelihood per token
    return -loglike.mean() / s


def compute_loss(input_ids, model):
    emiss_mat, prior, trans_mat = model(model.hidden_state_tokens)
    loss = forward_algo(em=emiss_mat, tm=trans_mat, p=prior, input_ids=input_ids)
    return loss
...
def fit(
    ...
    train: TrainArgs,
    eval: EvalArgs,
    hidden_state_tokens: torch.Tensor,
) -> None:
    model = state["model"]
    optimizer = state["optimizer"]
    model.hidden_state_tokens = hidden_state_tokens.to(model.device)
    ...
    #     meta_model = GPT(model.config, n_hidden_states=train.n_hidden_states)
    ...
    #     del meta_model, x
    measured_flops = 0 # TODO hacky fix
